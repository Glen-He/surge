...
describe("注册策略", () => {
  ...
  it("从数据库读取实时开关和邀请码策略", async () => {
    ...
    const { getRegistrationPolicy } = await import("@/lib/registration-policy");
  ...
  it("更新时关闭注册会同时关闭邀请码强制策略", async () => {
    ...
    const { updateRegistrationPolicy } = await import("@/lib/registration-policy");
    await expect(
  ...
  it("内部注册证明绑定邮箱并拒绝篡改", async () => {
    ...
    } = await import("@/lib/registration-policy");
