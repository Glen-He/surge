...
本项目的长期运行环境仅为云服务器，不维护常驻本地部署。需要验证数据库迁移、生产构建或浏览器流程时，使用隔离的临时数据库和临时报告目录，验证结束后立即清理；不得把 `reports_local/` 当作运行时数据目录或随本地部署删除。
...
7. **注册入口永久可见**：登录页必须始终同时显示“登录 / 注册”两个切换入口，不得根据环境变量、数据库注册策略、邀请码策略或其他运行状态条件渲染隐藏“注册”。关闭注册只允许禁止验证码发送和注册提交，并在注册面板内明确说明当前未开放；注册策略控制的是服务端注册权限，不是入口可见性。
