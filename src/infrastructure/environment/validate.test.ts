...
import { validateRuntimeEnvironment } from "@/infrastructure/environment/validate";
...
describe("生产内容域环境约束", () => {
  ...
  it("要求非本地主站配置独立 HTTPS 内容域", () => {
    ...
    expect(() => validateRuntimeEnvironment()).toThrow("REPORTS_ORIGIN");

    vi.stubEnv("REPORTS_ORIGIN", "https://glenhe.com");
    expect(() => validateRuntimeEnvironment()).toThrow("distinct from the main site");

    vi.stubEnv("REPORTS_ORIGIN", "http://reports.glenhe.com");
    expect(() => validateRuntimeEnvironment()).toThrow("must use HTTPS");
  });
