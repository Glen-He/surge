import { isNodejsRuntime } from "@/infrastructure/environment/server";

export async function register() {
  // Next 16 在 phase-production-build 不会调用 register，运行时密钥
  // （SMTP / MAINTENANCE / INVITE 等）因此不构成构建依赖；
  // 这里再按 runtime 显式分流，保证只有 Node.js 服务器实例执行启动序列。
  ...
  const { validateStorageSettings } = await import("@/features/reports/storage/storage-capacity");
  const { validateUploadGateSettings } = await import("@/features/reports/upload/upload-gate");
  ...
  const { ensureSchemaVersioned } = await import("@/infrastructure/database/migrations");
  ...
  const { validateDemoTemplates } = await import("@/features/auth/guest/guest-sandbox");
  await validateDemoTemplates();
  // 全量磁盘核对刻意不放在就绪路径上：调度器
  // （外部定时任务走 /api/internal/maintenance）在服务接收流量后、
  // 持有数据库租约时才执行。
  const { startMaintenanceScheduler } = await import("@/features/maintenance/scheduler");
